...
#
# - all the dependencies needed to run the TCF client code
#
...
#
# See also ../nreqs.Dockerfile
#
# Build as:
#
#  $ cd .../tcf.git
#  $ mkdir -p ~/tmp/cache/dnf  ~/tmp/cache/pkg
#  $ buildah bud -t tcf-client -v ~/tmp/dnf:/var/cache/dnf:rw  -v ~/tmp/pkg:/var/cache/PackageKit:rw  -v $PWD:/home/work/tcf.git:O --label version="$(git describe --always)" -f client.Dockerfile
#
...
RUN \
    ...
    DNF_COMMAND=dnf /home/work/tcf.git/nreqs.py install /home/work/tcf.git/base.nreqs.yaml; \
    dnf install -y \
        bind-utils \
        iputils \
        strace \
